...
**Status**: Implemented (debug.json output is a stub).
...
Global tracing uses the LangGraph callback handler. Session IDs are generated by
`config/langfuse.py` in the format `{team}-{dataset}-{timestamp}`.
